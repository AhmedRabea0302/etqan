                    ...
                    <table id="tree-table" class="table">
                        <tbody>
                            ...
                            @foreach($evidences as $evidence)
                                <tr data-id="{{$evidence->id}}" data-leaf="{{count($evidence->childrenEvidences) ? 0 : 1}}" data-parent="0" data-level="1" style="color: #232323;">
                                    <td data-column="name" data-leaf="{{count($evidence->childrenEvidences) ? 0 : 1}}" style=" border-top: none">
                                        @if(count($evidence->childrenEvidences))
                                ...
                                @if(count($evidence->childrenEvidences))
                                    @include('site.pages.evidences.evidence-partial',['sucpicionChilds' => $evidence->childrenEvidences, 'dataParent' => $evidence->id , 'dataLevel' => 1])
                                @endif
                            @endforeach
                        </tbody>
                    </table>
                            ...
                            <div class="col-md-12">
                                <div class="subject-content">
                                    ...
                                    <div class="subject-content-text">
                                        <p class="lead no-text text-center" style="margin-top: 180px">قم بإختيار دليل !</p>
                                    </div>
    ...
    <script>
        ...
        $('tr td').on('click', function(event) {
            ...
            if($leaf == 1 ) {
                ...
                $.ajax({
                    type: "GET",
                    url: '{{url('/evidence/')}}' + '/' + $id, // This is what I have updated
                }).done(function( data ) {
                    let suspicion = data;
        ...
        $('.add-comment-form form').submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: '{{url('evidence-add-comment')}}', // This is what I have updated
        ...
        let addReply = function(replyText, replytParent, replyCommentGetter) {
            ...
            $.ajax({
                type: "POST",
                url: '{{url('/evidence-add-reply')}}', // This is what I have updated
        ...
        $('.read-later').on('click', function(e) {
            e.preventDefault();
            $.ajax({
                url: '{{url('/read-evidence-later')}}/' + $id,
                type: 'POST',
                data: {_token: '{{ csrf_token()}}'}
            }).done((data) => {
                console.log(data);
        ...
        $('.share-content').on('click', function(e) {
            ...
            e.preventDefault();
            $.ajax({
                url: '{{url('/nosrah-evidence-content')}}/' + $id,
                type: 'GET',
            }).done(() => {
                window.open(`nosrah-evidence-content/${$id}`, '_blank');
            }).error(() => {
