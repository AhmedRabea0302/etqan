                    ...
                    <table id="tree-table" class="table">
                        <tbody>
                            ...
                            @foreach($suspicions as $suspicion)
                                <tr data-id="{{$suspicion->id}}" data-leaf="{{count($suspicion->childrenSuspicions) ? 0 : 1}}" data-parent="0" data-level="1" style="color: #232323;">
                                    <td data-column="name" data-leaf="{{count($suspicion->childrenSuspicions) ? 0 : 1}}" style=" border-top: none">
                                ...
                                @if(count($suspicion->childrenSuspicions))
                                    @include('site.pages.suspicions.suspicion-partial',['sucpicionChilds' => $suspicion->childrenSuspicions, 'dataParent' => $suspicion->id , 'dataLevel' => 1])
                                @endif
                            @endforeach
                        </tbody>
                    </table>
                        ...
                        <div class="col-md-12">
                            <div class="subject-content">
                                ...
                                <div class="subject-content-text">
                                    <p class="lead no-text text-center" style="margin-top: 180px">قم بإختيار شبهة !</p>
                                </div>
    ...
    <script>
        ...
        $('tr td').on('click', function(event) {
            ...
            if($leaf == 1 ) {
                ...
                $.ajax({
                    type: "GET",
                    url: '{{url('/hot-suspicion/')}}' + '/' + $id, // This is what I have updated
                }).done(function( data ) {
                    let suspicion = data;

                    // Set the subject header
                    subHeader.textContent = suspicion.suspicion;
        ...
        $(commentsIcon).on('click', function() {
            ...
            $.ajax({
                url: '{{url('/hot-suspicion-comments')}}' + '/' + $id,
                type: 'GET',
            }).done(function(data) {
                                ...
                                <p class="">
                                    <div class="comment-text">
                                        <div>
                                            ...
                                            <p class="comment-user-name">${c.user_name} <span> (${c.country} - ${c.city})</span></p>
                                        </div>
                                        <p class="comment">
                                            ${c.comment}
                                    ...
                                    <p class="">
                                        <div class="reply-text">
                                            <div>
                                                ...
                                                <p class="comment-user-name">${r.user_name} <span> (${r.country} - ${r.city})</span></p>    
                                            </div>
                                            ${r.reply}
                                            <p class="reply-date">
                                                <i class="fa fa-calendar"></i>
        ...
        $full = document.querySelector('.subject-content');
        fullButton = document.querySelector('.full-screen');
        fullButton.addEventListener('click', function(){
            // $full.style.backgroundColor = '#000';
        ...
        $('.add-comment-form form').submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: '{{url('hot-suspicion-add-comment')}}', // This is what I have updated
                ...
                comment.innerHTML = `
                    <div class="one-comment">
                        ...
                        <p class="comment-user-name">${data.user_name} <span> (${data.country} - ${data.city})</span></p>
                        <p class="">
                            <div class="comment-text">
                                ${data.comment}
        ...
        let addReply = function(replyText, replytParent, replyCommentGetter) {
            ...
            $.ajax({
                type: "POST",
                url: '{{url('/hot-suspicion-add-reply')}}', // This is what I have updated
        ...
        $('.share-content').on('click', function(e) {
            ...
            e.preventDefault();
            $.ajax({
                url: '{{url('/nosrah-hot-suspicion-content')}}/' + $id,
                type: 'GET',
            }).done(() => {
                window.open(`/nosrah-hot-suspicion-content/${$id}`, '_blank');
            }).error(() => {
