...
<div class="box-item">
    <div class="box-item-head">
        <h3 class="title">المناظرات المُضافة</h3>
    </div>
            ...
            <table id="tree-table" class="table">
                <tbody>
                    @foreach($discussions as $discussion)
                        <tr data-id="{{$discussion->id}}" data-parent="0" data-level="1">
                            <td data-column="name">
                        ...
                        @if(count($discussion->childrenDiscussions))
                            @include('pages.discussions.discussion-partial',['discussionChilds' => $discussion->childrenDiscussions, 'dataParent' => $discussion->id , 'dataLevel' => 1])
                        @endif      
                    @endforeach
                </tbody>
            </table>
...
<script>
    $(document).ready(function() {
        ...
        $('#tree-table .edit-button').on('click', function() {
           ...
           $('.modal .modal-footer input[type="hidden"]')[0].value = $id;
            ...
            $('.modal .modal-footer .button-update').on('click', function(event) {
                ...
                $data = { id: $id, title: $suspicionText}
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    type: "POST",
                    url: '{{route("post-update-title")}}',
                    data: $data,
        ...
        $('.modal .modal-footer .button-delete, .table .delete-button').on('click', function(event) {
            ...
            if (confirm('بحذفك هذه المناظرة ستقوم بحذف المناظرة و كل المناظرات المندرجة تحتها هل تريد الإستمرار؟')) {
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    type: "POST",
                    url: '{{route("delete-discussion")}}',
                    data: {id: $id},
                    success: function(response)
                    {  
