...
<div class="box-item">
    <div class="box-item-head">
        <h3 class="title">الشبُهات الساخنة المُضافة</h3>
    </div>
            ...
            <table id="tree-table" class="table">
                <tbody>
                    @foreach($suspicions as $suspicion)
                        ...
                        @if(count($suspicion->childrenSuspicions))
                            @include('pages.hot-suspicions.suspicion-hot-partial',['sucpicionChilds' => $suspicion->childrenSuspicions, 'dataParent' => $suspicion->id , 'dataLevel' => 1])
...
<script>
    $(document).ready(function() {
        ...
        $('#tree-table .edit-button').on('click', function() {
            ...
            $('.modal .modal-footer .button-update').on('click', function(event) {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: '{{route("post-update-hot-suspicion-title")}}',
                    data: $data,
        ...
        $('.modal .modal-footer .button-delete, .table .delete-button').on('click', function(event) {
            ...
            if (confirm('بحذفك هذه الشبهة الساخنة ستقوم بحذف الشبهة و كل الشبهات المندرجة تحتها هل تريد الإستمرار؟')) {
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    type: "POST",
                    url: '{{route("delete-hot-suspicion")}}',
                    data: {id: $id},
                    success: function(response)
                    {  
