...
<div class="box-item">
    <div class="box-item-head">
        <h3 class="title">الأدلة المُضافة</h3>
    </div>
            ...
            <table id="tree-table" class="table">
                <tbody>
                    @foreach($evidences as $evidence)
                        <tr data-id="{{$evidence->id}}" data-parent="0" data-level="1">
                            <td data-column="name">
                        ...
                        @if(count($evidence->childrenevidences))
                            @include('pages.evidences.evidence-partial',['evidenceChilds' => $evidence->childrenevidences, 'dataParent' => $evidence->id , 'dataLevel' => 1])
                            {{-- @include('pages.evidences.suspicion-reply',['suspicion' => $suspicion, 'dataParent' => $suspicion->id]) --}}
                        @endif      
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="modal-dialog box-item">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body  box-item-content">
                ...
                <textarea class="form-control" name="title"  id="" cols="30" rows="3"></textarea>
                <br>
...
<script>
    $(document).ready(function() {
        ...
        $('#tree-table .edit-button').on('click', function() {
            ...
            $('.modal .modal-body textarea[name="title"]').val($suspicion);
            ...
            $('.modal .modal-footer .button-update').on('click', function(event) {
                ...
                var $suspicionText = '';
                $suspicionText = $('.modal .modal-body textarea[name="title"]').val().trim();
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: '{{route("post-update-evidence-title")}}',
                    data: $data,
        ...
        $('.modal .modal-footer .button-delete, .table .delete-button').on('click', function(event) {
            ...
            if (confirm('بحذفك هذا الدليل ستقوم بحذف الدليل و كل الأدلة المندرجة تحته هل تريد الإستمرار؟')) {
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    type: "POST",
                    url: '{{route("delete-evidence")}}',
                    data: {id: $id},
                    success: function(response)
                    {  
