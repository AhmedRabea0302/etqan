...
class SuspicionController extends Controller
{
    public function getAddSuspicion() {
        ...
        $suspicions = Suspicion::where('parent_id', '0')->with('childs')->get();
        $allSuspicions = Suspicion::select('id', 'suspicion')->where('level','!=' , 1)->get();
        return view('pages.suspicions.add-suspicion', ['suspicions' => $suspicions, 'allSuspicions' => $allSuspicions]);
    }
    ...
    public function AddSuspicion(Request $request) {
        ...
        if(!empty($file1)) {
            ...
            $destination = 'storage/uploads/videos/suspicions';
            if($suspicion->video_url) {
                unlink($suspicion->video_url);
            }
            $file1->move($destination, $file_name);
            $suspicion->video_url = url('/storage/uploads/videos/suspicions/') . '/' . $file_name;
        }

        if(!empty($file2)) {
            $file_name = time() . '342.' . $file2->getClientOriginalExtension();
           
            $destination = 'storage/uploads/books/suspicions';
            if($suspicion->book_url) {
                unlink($suspicion->book_url);
            }
            $file2->move($destination, $file_name);
            $suspicion->book_url = url('/storage/uploads/books/suspicions/') . '/' . $file_name;
        }

        $suspicion->save();

        session()->push('m', 'success');
        session()->push('m', 'تم إضافة الشبُهة بنجاح');
        return redirect()->back();
    }

    public function getAllSuspicions() {
        // $suspicions = Suspicion::where('parent_id', '0')->get();
        $suspicions = Suspicion::select('id', 'parent_id','suspicion')->where('parent_id', '0')->with('childrenSuspicions')->get();
        return view('pages.suspicions.all-suspicions')->with(['suspicions' => $suspicions]);
    }
    ...
    public function getSuspicionContent($id) {
        ...
        $book_name = end($arr);
        return view('pages.suspicions.suspicion-content')->with(['suspicion' => $suspicion,'book_name' => $book_name] );
    }
    ...
    public function updateSuspicion(Request $request, $id) {
        ...
        if(!empty($file1)) {
            ...
            $file1->move($destination, $file_name);
            $suspicion->video_url = 'https://nusratelsunnahacademy.com/nosrah/storage/uploads/videos/suspicions/' . $file_name;
        }
        ...
        if(!empty($file2)) {
            ...
            $file2->move($destination, $file_name);
            $suspicion->book_url = 'https://nusratelsunnahacademy.com/nosrah/storage/uploads/books/suspicions/' . $file_name;
        }
    ...
    public function updateSuspicionTitle(Request $request) {
        ...
        if($suspicions) {
            foreach($suspicions as $suspicion) {
                if($suspicion->book_url) {
                    ...
                    unlink('storage/uploads/books/suspicions/' . $book_name);
                }

                if($suspicion->video_url) {
                    $video_arr = explode('/', $suspicion->video_url);
                    $video_name = end($video_arr);
                    unlink('storage/uploads/videos/suspicions/' . $video_name);
                }
                $suspicion->delete();
            }
        }
