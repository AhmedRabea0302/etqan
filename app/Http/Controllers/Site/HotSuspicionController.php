...
class HotSuspicionController extends Controller
{
    public function index() {
        ...
        $suspicions = HotSuspicion::select('id', 'parent_id','suspicion', 'short_reply')->where('parent_id', '0')->with('childrenSuspicions')->get();
        return view('site.pages.hot-suspicions.index')->with('suspicions', $suspicions);
    }
    ...
    public function getSuspicionComments($id) {
        ...
        $comments = HotSuspicionComment::select('id', 'user_id', 'user_type', 'comment', 'created_at')->where('subject_id', $id)->orderBy('created_at', 'ASC')->get();
    ...
    public function getReplies($commentId) {
        $replies = HotSuspicionReply::select('id', 'user_id', 'user_type', 'reply', 'created_at')->where('comment_id', $commentId)->orderBy('created_at', 'ASC')->get();
        if(count($replies)) {
    ...
    public function addCommnet(Request $request) {
        $comment = new HotSuspicionComment();
        $rules = [
            'comment'       => 'required'
        ];        
        $messages = [
    ...
    public function addToFavorite($id) {
        ...
        $checkFav = DB::select('select * from favorite_subjects where(user_id = :user_id AND subject_id = :sub_id AND type = "hot-suspicions")', ['user_id' => $user_id, 'sub_id' => $subject_id]);
        if($checkFav) {
    ...
    public function readLater($id) {
        $user_id = auth()->guard('auth-site')->user()->id;
        $subject_id = number_format($id);
        $readLater = DB::select('select * from read_laters where(user_id = :user_id AND subject_id = :sub_id AND type = "hot-suspicions")', ['user_id' => $user_id, 'sub_id' => $subject_id]);
        if($readLater) {
    ...
    public function getSuspicionContentShare($id) {
        ...
        return view('site.pages.hot-suspicions.hot-suspicion-content-share')->
